...
describe('fileSkill', () => {
	...
	describe('parameter validation', () => {
		it('should throw error if action is missing', async () => {
			await expect(fileSkill.execute({ path: './test.txt' })).rejects.toThrow('Missing required parameter: action');
		});

		it('should throw error if action is invalid', async () => {
			await expect(fileSkill.execute({ action: 'invalid', path: './test.txt' })).rejects.toThrow('Invalid action');
		});

		it('should throw error if path is missing', async () => {
			await expect(fileSkill.execute({ action: 'read' })).rejects.toThrow('Missing required parameter: path');
		});

		it('should throw error if content is missing for write action', async () => {
			await expect(fileSkill.execute({ action: 'write', path: './test.txt' })).rejects.toThrow('Missing required parameter for write action: content');
		});
	});
	...
	describe('error handling', () => {
		it('should throw error if execute is called with undefined params', async () => {
			// @ts-expect-error
			await expect(fileSkill.execute(undefined)).rejects.toThrow();
		});
		it('should throw error if execute is called with null params', async () => {
			// @ts-expect-error
			await expect(fileSkill.execute(null)).rejects.toThrow();
		});
		it('should throw error if execute is called with empty object', async () => {
			await expect(fileSkill.execute({})).rejects.toThrow('Missing required parameter: action');
		});
	});
});
